...
export default function Register() {
  const { register, connectWallet, web3Register } = useAuth();
  const navigate = useNavigate();
  
  // Registration Type Toggle
  const [authMode, setAuthMode] = useState('thirdweb'); // 'thirdweb' or 'standard'
  ...
  const handleThirdwebRegister = async (type) => {
    setError('');
    if (!formData.name) {
      setError('Please fill in your Full Name to register.');
      return;
    }
    setWeb3Loading(true);
    ...
    try {
      ...
      if (type === 'email') {
        ...
        setTimeout(async () => {
          if (res.success) {
            const regRes = await web3Register(res.address, formData, 'embedded');
            if (regRes.success) {
              navigate('/chat');
            } else {
      ...
      // External Wallet (Core / MetaMask)
      res = await connectWallet();
      if (res.success) {
        const regRes = await web3Register(res.address, formData, 'metamask');
        if (regRes.success) {
          navigate('/chat');
        } else {
          setError(regRes.error);
      ...
      setError(e.message || 'Web3 Registration failed.');
    } finally {
      if (type !== 'email') {
        setWeb3Loading(false);
      }
  ...
  const handleSocialRegister = async (provider, emailVal) => {
    if (!formData.name) { setError('Please fill in your Full Name first.'); setSocialModalOpen(false); return; }
    if (!emailVal || !emailVal.trim() || !emailVal.includes('@')) {
      setError('Please enter a valid email address.');
      setSocialModalOpen(false);
      return;
    }
    setError(''); setSocialLoadingType(provider);
    setSocialModalOpen(false);
    try {
